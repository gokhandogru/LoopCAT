...
export function installApplicationComposition({ appRuntime, browserGlobals, compatibilityModules, window }) {
  ...
  const {
    validateProjectPackage: validatePackage,
    validateBackupFile,
    planDeliveryExport,
    validateExportReadiness,
    reportCount,
    reportSummary
  } = compatibilityModules.validation;
  const { analyzeProject } = compatibilityModules.analysis;
  const {
    buildQualityPassportData,
    buildRiskQueue,
    defaultQualityProfile,
    qualityCategoryLabel: baseQualityCategoryLabel,
    scoreSegment
  } = compatibilityModules.quality;
  const {
  ...
  const { openAi: OPENAI_KEY_STORAGE, localAiLegacy: LOCAL_AI_KEY_STORAGE } =
  ...
  const applicationSaveStatusController = appRuntime.featureFactories.createApplicationSaveStatusController({
    redaction: { sanitize: applicationTextSafetyService.redactSensitiveText },
  ...
  workspaceRecoveryPresentationService = appRuntime.featureFactories.createWorkspaceRecoveryPresentationService({
  ...
  const revisionHistoryPresentationService = appRuntime.featureFactories.createRevisionHistoryPresentationService({
  ...
  const revisionHistoryRenderScheduler = appRuntime.featureFactories.createRevisionHistoryRenderScheduler({
  ...
  const segmentStatusPresentationService = appRuntime.featureFactories.createSegmentStatusPresentationService({
    document,
  ...
  const segmentRowPresentationService = appRuntime.featureFactories.createSegmentRowPresentationService({
  ...
  const segmentProgressPresentationService = appRuntime.featureFactories.createSegmentProgressPresentationService({
  ...
  const projectDeletionController = appRuntime.featureFactories.createProjectDeletionController({
    ...
    test: {
      ...
      documentDeleteFails: (documentInfo) => Boolean(LOOPCAT_TEST_BUILD && documentInfo[FILE_DELETE_FAILURE_TEST_FLAG]),
  ...
  const projectsViewPresentationController = appRuntime.featureFactories.createProjectsViewPresentationController({
    elements: {
  ...
  const editorShellPresentationController = appRuntime.featureFactories.createEditorShellPresentationController({
    elements: {
  ...
  const projectHomePresentationController = appRuntime.featureFactories.createProjectHomePresentationController({
    elements: {
  ...
  const documentFilterPresentationController = appRuntime.featureFactories.createDocumentFilterPresentationController({
  ...
  const projectListPresentationController = appRuntime.featureFactories.createProjectListPresentationController({
  ...
  const projectDocumentOpenController = appRuntime.featureFactories.createProjectDocumentOpenController({
    session: {
      getProject: editorSessionStore.getProject,
      getSegments: editorSessionStore.getSegments
    },
  ...
  const applicationCommandHistoryController = appRuntime.featureFactories.createApplicationCommandHistoryController({
  ...
  const segmentConfirmationStateService = appRuntime.featureFactories.createSegmentConfirmationStateService({
  ...
  const projectDocumentManifestService = appRuntime.featureFactories.createProjectDocumentManifestService({
    session: { getProject: editorSessionStore.getProject },
  ...
  segmentGridPresentationController = appRuntime.featureFactories.createSegmentGridPresentationController({
    document,
    body: els.segmentBody,
    viewport: els.segmentGridWrap,
  ...
  const localizationDownloadMimeTypeService = appRuntime.featureFactories.createLocalizationDownloadMimeTypeService({
  ...
  const projectResourceTransferController = appRuntime.featureFactories.createProjectResourceTransferController({
    session: { getProject: editorSessionStore.getProject },
    files: {
  ...
  const aiCredentialStorageService = appRuntime.featureFactories.createAiCredentialStorageService({
    ...
    failures: {
      beforeOpenAiSave: () => LOOPCAT_TEST_BUILD && state[OPENAI_KEY_STORAGE_FAILURE_TEST_FLAG]
    },
    logger: console
  });
  const aiRuntimeSettingsService = appRuntime.featureFactories.createAiRuntimeSettingsService({
  ...
  const aiLocalSettingsPersistenceController = appRuntime.featureFactories.createAiLocalSettingsPersistenceController({
    editorSessionStore,
    ...
    settings: {
      ...
      projectUpdateFields: (settings, project) => localAISettingsStore.projectUpdateFields(settings, project)
    },
  ...
  const aiProviderPresentationService = appRuntime.featureFactories.createAiProviderPresentationService({
    providers: {
  ...
  const aiProviderFormController = appRuntime.featureFactories.createAiProviderFormController({
  ...
  const aiSuggestionListController = appRuntime.featureFactories.createAiSuggestionListController({
  ...
  const aiPretranslationController = appRuntime.featureFactories.createAiPretranslationController({
    ...
    settings: {
      ...
      projectDefaults: (project) => aiRuntimeSettingsService.normalizeProjectSettings(project.aiSettings)
    },
    providers: {
      get: (settings = aiRuntimeSettingsService.localSettingsFromForm()) => aiProviderService.get(settings.providerId),
    ...
    activity: {
      log: (details) => projectActivityController.log("ai-pretranslate", "Local AI pretranslation applied", details)
    },
    workspace: { markDirty: workspaceDirtyStateController.mark },
    status: { set: applicationSaveStatusController.set },
    testHooks: {
  ...
  const aiReviewController = appRuntime.featureFactories.createAiReviewController({
    ...
    activity: {
      logActive: (details) => projectActivityController.log("ai-review", "AI segment review created", details),
      logBatch: (details) => projectActivityController.log("ai-batch-review", "Batch AI QA completed", details)
    },
    workspace: { markDirty: workspaceDirtyStateController.mark },
    status: { set: applicationSaveStatusController.set },
  ...
  const aiTagRepairController = appRuntime.featureFactories.createAiTagRepairController({
    ...
    activity: {
      logBatch: (details) =>
        projectActivityController.log("ai-tag-repair-batch", "Batch AI tag repair suggestions created", details)
    },
    workspace: { markDirty: workspaceDirtyStateController.mark },
    status: { set: applicationSaveStatusController.set },
    redact: applicationTextSafetyService.redactSensitiveText,
  ...
  const aiAlternativesController = appRuntime.featureFactories.createAiAlternativesController({
    ...
    activity: {
      ...
      logBatch: (details) =>
        projectActivityController.log("ai-target-variants-batch", "Batch AI target alternatives created", details)
    },
    workspace: { markDirty: workspaceDirtyStateController.mark },
    status: { set: applicationSaveStatusController.set },
    redact: applicationTextSafetyService.redactSensitiveText,
  ...
  const aiTerminologyApplicationController = appRuntime.featureFactories.createAiTerminologyApplicationController({
    editorSessionStore,
    selection: {
      getActiveSegment: applicationActiveSegmentService.get,
      getActiveIndex: () => applicationStore.getState().navigation.activeIndex
    ...
    activity: {
      logBatch: (details) =>
        projectActivityController.log("ai-apply-terminology-batch", "Batch AI terminology suggestions created", details)
    },
    workspace: { markDirty: workspaceDirtyStateController.mark },
    status: { set: applicationSaveStatusController.set },
    redact: applicationTextSafetyService.redactSensitiveText,
    ...
    suggestions: {
      append: (operation, segment, suggestion) =>
        aiSuggestionPersistenceController.append(
          ...
          operation === "adapt" ? "AI draft adaptation suggestion created" : "AI draft polish suggestion created"
        ),
      normalize: (...args) => aiSuggestionPersistenceController.normalize(...args),
      nextId: () => makeId("ai-suggestion")
    },
  ...
  const aiTermCandidatePersistenceService = appRuntime.featureFactories.createAiTermCandidatePersistenceService({
    project: { get: editorSessionStore.getProject },
    termbase: {
      list: listTerms,
      save: saveTerm
  ...
  const aiTerminologyExtractionController = appRuntime.featureFactories.createAiTerminologyExtractionController({
    editorSessionStore,
    selection: { getActiveSegment: applicationActiveSegmentService.get },
    scope: {
    ...
    termbase: {
      ...
      saveCandidates: (terms, termBaseName) => aiTermCandidatePersistenceService.saveCandidates(terms, termBaseName)
    },
    settings: {
      persist: aiLocalSettingsPersistenceController.persistSilently,
      runtimeConfig: aiRuntimeSettingsService.runtimeConfig,
    ...
    activity: {
      ...
      logBatch: (details) =>
        projectActivityController.log("ai-term-extraction-batch", "Batch AI term candidates extracted", details)
    },
    workspace: { markDirty: workspaceDirtyStateController.mark },
    status: { set: applicationSaveStatusController.set },
    logger: console
  ...
  const aiProjectBriefController = appRuntime.featureFactories.createAiProjectBriefController({
    ...
    activity: {
      log: (details) => projectActivityController.log("ai-project-brief", "AI project brief generated", details)
    },
    workspace: { markDirty: workspaceDirtyStateController.mark },
    status: { set: applicationSaveStatusController.set },
    logger: console
  });
  const aiSuggestionApplicationController = appRuntime.featureFactories.createAiSuggestionApplicationController({
    editorSessionStore,
    commands: {
      bus: appRuntime.commands.bus,
    ...
    activity: {
      log: (details) => projectActivityController.log("ai-apply-suggestion", "AI suggestion applied to target", details)
    },
    presentation: {
      renderSegments: segmentGridPresentationController.render,
      renderProgress: segmentProgressPresentationService.render,
  ...
  const aiSuggestionPersistenceController = appRuntime.featureFactories.createAiSuggestionPersistenceController({
    mutation: {
      touch: segmentTargetStateService.touch,
      restoreInPlace: (segment, snapshot) => {
        Reflect.ownKeys(segment).forEach((key) => delete segment[key]);
  ...
  const aiSettingsPersistenceController = appRuntime.featureFactories.createAiSettingsPersistenceController({
    editorSessionStore,
    forms: {
    ...
    settings: {
      normalize: aiRuntimeSettingsService.normalizeProjectSettings,
      projectUpdateFields: (localSettings, project) => localAISettingsStore.projectUpdateFields(localSettings, project)
    },
    endpoint: { assertAllowed: aiRuntimeSettingsService.assertEndpointAllowed },
  ...
  const aiProviderAdministrationOperationsController =
    appRuntime.featureFactories.createAiProviderAdministrationOperationsController({
      ...
      desktop: {
        getBridge: () =>
          window.LoopCATDesktop && typeof window.LoopCATDesktop.startLmStudioServer === "function"
  ...
  const segmentActionButtonsController = appRuntime.featureFactories.createSegmentActionButtonsController({
    elements: {
  ...
  const applicationCommandButtonsController = appRuntime.featureFactories.createApplicationCommandButtonsController({
    elements: {
  ...
  const applicationOfflineShellController = appRuntime.featureFactories.createApplicationOfflineShellController({
    browser: {
  ...
  const applicationUpdateControlsController = appRuntime.featureFactories.createApplicationUpdateControlsController({
    elements: {
      reloadButton: els.reloadUpdateBtn,
      deferButton: els.deferUpdateBtn
    },
  ...
  const uiLocaleOrchestrationController = appRuntime.featureFactories.createUiLocaleOrchestrationController({
    elements: {
  ...
  const projectFilterControlsController = appRuntime.featureFactories.createProjectFilterControlsController({
    elements: {
      searchInput: els.projectSearchInput,
      languagePairFilter: els.languagePairFilter
    },
  ...
  const editorFilterControlsController = appRuntime.featureFactories.createEditorFilterControlsController({
    elements: {
  ...
  const applicationEventWiringController = appRuntime.featureFactories.createApplicationEventWiringController({
    checkpoint: (message) => {
  ...
  const applicationStartupController = appRuntime.featureFactories.createApplicationStartupController({
    ...
    // @ts-expect-error The test renderer injects this lexical function at the checked marker below.
  ...
  const projectExportBuildService = appRuntime.featureFactories.createProjectExportBuildService({
    session: {
      getProject: editorSessionStore.getProject,
      getSegments: editorSessionStore.getSegments
    },
  ...
  workspaceBackupExportController = appRuntime.featureFactories.createWorkspaceBackupExportController({
    connection: {
  ...
  workspaceHealthRepairController = appRuntime.featureFactories.createWorkspaceHealthRepairController({
    connection: {
      isConnected: () => Boolean(workspaceStorage && state.workspaceStatus?.connected)
    },
    storage: {
  ...
  workspacePackageSaveController = appRuntime.featureFactories.createWorkspacePackageSaveController({
    storage: {
  ...
  const projectImportRestoreController = appRuntime.featureFactories.createProjectImportRestoreController({
    files: {
  ...
  const projectDocumentImportController = appRuntime.featureFactories.createProjectDocumentImportController({
    session: editorSessionStore,
    catalog: {
  ...
  const projectDialogSaveController = appRuntime.featureFactories.createProjectDialogSaveController({
    form: {
  ...
  const tmPretranslationDialogController = appRuntime?.featureFactories?.createTmPretranslationDialogController?.({
    ...
    scheduleFrame: requestAnimationFrame,
    onError: (error) =>
      applicationSaveStatusController.set(error?.message || "TM pretranslation settings could not be opened.", "dirty")
  });
  ...
  const resourceLibraryImportController = appRuntime.featureFactories.createResourceLibraryImportController({
    forms: {
  ...
  const reviewStateController = appRuntime.featureFactories.createReviewStateController({
    ...
    restoration: {
      restoreCommand: (segmentId, snapshot) => segmentCommandRestorationController.restoreSnapshot(segmentId, snapshot)
    },
    activity: {
