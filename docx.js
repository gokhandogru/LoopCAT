...
function runPropertiesElement(rPrXml) {
  ...
  return Array.from(wrapper.documentElement.childNodes).find((child) => child.nodeType === Node.ELEMENT_NODE && child.localName === "rPr") || null;
}
...
function hasOnlySemanticRunProperties(rPrXml) {
  ...
  return Boolean(children.length) && children.every((child) => allowed.has(child.localName));
}
...
function ensureRunPropertiesXml(baseRPrXml, semanticTags = []) {
  ...
  const runProperties = Array.from(xml.documentElement.childNodes).find((child) => child.nodeType === Node.ELEMENT_NODE && child.localName === "rPr");
