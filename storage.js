...
function normalizeProjectResourceLinks(project = {}, mainTmName = "Default TM") {
  ...
  rawLinks.forEach((link, index) => {
    ...
    links.push({
      ...link,
      id: typeof link.id === "string" && link.id.trim() ? link.id : `${project.id || "project"}-${type}-link-${index + 1}`,
      type,
      name,
  ...
  if (!links.some((link) => link.type === "termbase")) {
    links.push({ id: `${project.id || "project"}-tb-link`, type: "termbase", name: cleanPortableLabel(project.termBaseName, "Default TB") });
  }
  return links;
}
...
function normalizeProject(project = {}) {
  ...
  const mainTmName = cleanPortableLabel(projectWithoutAcademicMetadata.mainTmName, cleanPortableLabel(projectWithoutAcademicMetadata.tmName, "Default TM"));
  ...
  return {
    ...
    tmName: mainTmName,
    mainTmName,
    termBaseName: resourceLinks.find((link) => link.type === "termbase")?.name || cleanPortableLabel(projectWithoutAcademicMetadata.termBaseName, "Default TB"),
...
function normalizeSegment(segment) {
  return {
    ...
    aiSuggestions: Array.isArray(segment.aiSuggestions) ? segment.aiSuggestions.map((item) => sanitizedAiSuggestion(item)) : [],
...
function sanitizePortableValue(value, key = "", path = [], context = null) {
  ...
  if (isPortableLabelPath(currentPath) && (typeof value === "string" || typeof value === "number")) return redactSensitiveText(value);
  if (isPortableRecordIdPath(currentPath) && (typeof value === "string" || typeof value === "number")) return cleanPortableRecordId(value, "", activeContext);
  ...
  if (isTmEntryLike(value) && !isSourceJsonPath(currentPath)) return sanitizedTmEntryRecord(value, currentPath, activeContext);
  if (isTermRecordLike(value) && !isSourceJsonPath(currentPath)) return sanitizedTermRecord(value, currentPath, activeContext);
  if (value && typeof value === "object") {
    const clean = {};
...
function assertSegmentsBelongToRestoredProjects(segments = [], projects = []) {
  ...
  const preview = orphaned.slice(0, 3).map((segment) => segment.id || segment.projectId).join(", ");
  throw new Error(`${orphaned.length} segment${orphaned.length === 1 ? "" : "s"} belong to projects not present in the backup${preview ? `: ${preview}` : ""}.`);
}
...
function assertActivityEventsBelongToRestoredProjects(activityEvents = [], projects = []) {
  ...
  if (!orphaned.length) return;
  const preview = orphaned.slice(0, 3).map((event) => event.id || event.projectId).join(", ");
  throw new Error(`${orphaned.length} activity event${orphaned.length === 1 ? "" : "s"} belong to projects not present in the backup${preview ? `: ${preview}` : ""}.`);
}
  ...
  (projects || []).forEach((project, projectIndex) => {
    ...
    project.documents.forEach((documentInfo, documentIndex) => {
      if (!documentInfo || typeof documentInfo !== "object") {
        throw new Error(`${label} ${project.id || projectIndex + 1} document ${documentIndex + 1} manifest entry must be an object.`);
      }
      if (!documentInfo.id) return;
      if (documentIds.has(documentInfo.id)) throw new Error(`Duplicate document ID in project manifest: ${documentInfo.id}.`);
  ...
  (projects || []).forEach((project, projectIndex) => {
    ...
    project.resourceLinks.forEach((link, linkIndex) => {
      ...
      if (link.id !== undefined) {
        if (typeof link.id !== "string" || !link.id.trim()) throw new Error(`${linkLabel} ID must be a non-empty string.`);
    ...
    if (mainTmLinks > 1) throw new Error(`${projectLabel} resource links contain multiple main translation memories.`);
  });
}
...
function assertSegmentsBelongToProjectDocuments(segments = [], projects = [], label = "backup") {
  ...
  const orphaned = (segments || []).filter((segment) => (
    segment?.projectId &&
    segment?.documentId &&
  ...
  if (!orphaned.length) return;
  const preview = orphaned.slice(0, 3).map((segment) => segment.id || segment.documentId).join(", ");
  throw new Error(`${orphaned.length} ${label} segment${orphaned.length === 1 ? "" : "s"} refer to documents not present in their project manifest${preview ? `: ${preview}` : ""}.`);
}
...
function assertPackageRecordsBelongToProject(project, segments = [], activityEvents = []) {
  ...
  const mismatchedSegments = (segments || []).filter((segment) => segment?.projectId && segment.projectId !== projectId);
  if (mismatchedSegments.length) {
    const preview = mismatchedSegments.slice(0, 3).map((segment) => segment.id || segment.projectId).join(", ");
    throw new Error(`${mismatchedSegments.length} project package segment${mismatchedSegments.length === 1 ? "" : "s"} belong to a different project${preview ? `: ${preview}` : ""}.`);
  }
  const mismatchedEvents = (activityEvents || []).filter((event) => event?.projectId && event.projectId !== projectId);
  if (mismatchedEvents.length) {
    const preview = mismatchedEvents.slice(0, 3).map((event) => event.id || event.projectId).join(", ");
    throw new Error(`${mismatchedEvents.length} project package activity event${mismatchedEvents.length === 1 ? "" : "s"} belong to a different project${preview ? `: ${preview}` : ""}.`);
  }
...
async function bulkPutIfRevisionNotOlder(storeName, values = [], revisionField = "revision") {
  ...
  await Promise.all(values.map((value, index) => new Promise((resolve, reject) => {
    const read = store.get(value.id);
    read.onsuccess = () => {
      const existing = read.result;
      const existingRevision = Number(existing?.[revisionField] || 0);
...
async function deleteStoresWhereAtomically(predicatesByStore) {
  const storeNames = Object.keys(predicatesByStore).filter((storeName) => typeof predicatesByStore[storeName] === "function");
  if (!storeNames.length) return predicatesByStore;
  const db = await openDatabase();
  const tx = db.transaction(storeNames, "readwrite");
  await Promise.all(storeNames.map((storeName) => deleteWhereInStore(tx.objectStore(storeName), predicatesByStore[storeName])));
  await txDone(tx);
  return predicatesByStore;
}

async function importProjectPackageRecords({ project, segments = [], tmEntries = [], terms = [], activityEvents = [], replaceProjectId = "" }) {
  ...
  const importedActivityEvents = portableRecordArray(activityEvents, "Project package activity events", portableContext);
  ...
  const [segmentKeysToDelete, activityKeysToDelete, existingProject, existingSegments, existingActivityEvents, existingTmEntries, existingTerms] = replaceProjectId
    ? await Promise.all([
  ...
  await txDone(tx);
  return { project: importedProject, segments: importedSegments, tmEntries: importedTmEntries, terms: importedTerms, activityEvents: importedActivityEvents };
}
...
async function deleteProjectRecords(projectId) {
  ...
  const activityDelete = deleteWhereInStore(tx.objectStore("activityEvents"), (event) => event.projectId === projectId);
...
async function updateProjectAndDeleteDocumentSegments(project, documentId) {
  ...
  const segmentDelete = deleteWhereInStore(tx.objectStore("segments"), (segment) => segment.projectId === project.id && segment.documentId === documentId);
...
async function exportAllData() {
  const [projects, segments, tmEntries, terms, activityEvents] = await Promise.all([
    getAll("projects"),
    getAll("segments"),
    getAll("tmEntries"),
    getAll("terms"),
...
async function importAllData(data) {
  ...
  if (Number.isFinite(schemaVersion) && schemaVersion > DB_VERSION) {
    throw new Error(`Backup schema version ${schemaVersion} is newer than this LoopCAT build supports. Update LoopCAT before restoring this backup.`);
  }
  const portableContext = createPortableSanitizerContext();
...
window.CatHan.storage = {
  ...
  constants: { LOCAL_WORKSPACE_ID, LOCAL_USER_ID, SCHEMA_VERSION: DB_VERSION },
  exportAllData,
  importAllData
};
})();
