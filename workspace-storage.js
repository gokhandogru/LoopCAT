(() => {
  ...
  const BACKUP_SCHEMA_VERSION = window.CatHan.storage.constants.BACKUP_SCHEMA_VERSION;
  ...
  function cleanPathPart(value, fallback = "item") {
    return (
      workspaceSafeLabel(value, fallback)
        .normalize("NFKD")
  ...
  function unsafeWorkspacePathPart(value) {
    ...
    return (
      ...
      part === ".." ||
      /[\u0000-\u001f\u007f/\\]/.test(part) ||
      SENSITIVE_TEXT_VALUE_PATTERN.test(part)
    );
  }
  ...
  function defaultManifest(existing = {}) {
    ...
    return {
      ...
      projects: Array.isArray(existing.projects)
        ? existing.projects.map(sanitizeManifestProjectRef).filter(Boolean)
        : [],
      resources: Array.isArray(existing.resources)
        ? existing.resources.map(sanitizeManifestResourceRef).filter(Boolean)
        : [],
  ...
  function workspaceJsonTooLargeError(label, size) {
    const error = new Error(
      `${label} is too large to read from the workspace folder. Keep LoopCAT JSON files under 50 MB.`
    );
  ...
  async function writeJson(pathParts, value) {
    ...
    try {
      ...
      lastError = workspaceSafeLabel(
        ...
        "Could not write workspace JSON."
      );
  ...
  function isMissingOrInvalidJson(error) {
    return (
      error.name === "NotFoundError" || error.name === "SyntaxError" || error.name === "WorkspaceJsonTooLargeError"
    );
  }

  function workspacePackageReadWarning(folderName, error) {
    const reason = workspaceSafeLabel(
      error.message || error.name || "The package JSON could not be read.",
      "The package JSON could not be read."
    );
  ...
  function workspacePackageValidationReason(validation) {
    return workspaceSafeLabel(
      (validation?.errors || []).filter(Boolean).join("; "),
      "Project package failed validation."
    );
  }
  ...
  function validateWorkspaceProjectPackage(pkg, label = "Workspace project package") {
    ...
    if (validation && !validation.ok) {
      const error = new Error(
        `${workspaceSafeLabel(label, "Workspace project package")} failed validation: ${workspacePackageValidationReason(validation)}`
      );
  ...
  function validateWorkspaceBackupFile(data, label = "Workspace backup") {
    ...
    if (validation && !validation.ok) {
      const error = new Error(
        `${workspaceSafeLabel(label, "Workspace backup")} failed validation: ${workspaceBackupValidationReason(validation)}`
      );
  ...
  function sanitizeManifestProjectRef(projectRef) {
    ...
    const fallbackProjectPath = [
      PROJECT_DIR,
      `${cleanPathPart(projectRef.name || projectRef.id, "project")}-${shortId(projectRef.id)}`,
      PACKAGE_FILE
    ];
    return {
      ...
      schemaVersion: Number.isFinite(Number(projectRef.schemaVersion))
        ? Number(projectRef.schemaVersion)
      ...
      resourceRefs: Array.isArray(projectRef.resourceRefs)
        ? projectRef.resourceRefs.map(sanitizeManifestResourceRef).filter(Boolean)
        : []
    };
  }
  ...
  function packageTimestamp(pkg) {
    ...
    return (
      new Date(
        pkg?.packageMetadata?.savedAt || pkg?.savedAt || pkg?.exportedAt || project.updatedAt || project.createdAt || 0
      ).getTime() || 0
    );
  }
  ...
  async function scanProjectPackages() {
    ...
    for (const [folderName, entryHandle] of await directoryEntries(projectsDir)) {
      ...
      try {
        ...
        if (pkg?.project?.id) {
          packages.push({
            pkg: withWorkspaceValidation(pkg, validation),
            packagePath: [PROJECT_DIR, folderName, PACKAGE_FILE]
          });
        }
      } catch (error) {
  ...
  async function rebuildManifestFromPackages(existing = {}) {
    ...
    const recoveredResources = discovered.flatMap(({ pkg }) => summarizePackageResources(pkg));
  ...
  function resourceKey(resource) {
    return [resource.type, resource.name, resource.sourceLang || "", resource.targetLang || ""].join("::");
  }
  ...
  function summarizePackageResources(pkg) {
    ...
    (pkg.resources?.tmEntries || []).forEach((entry) => {
      const resource = {
        ...
        languagePair:
          entry.languagePair ||
          `${entry.sourceLang || project.sourceLang || ""}::${entry.targetLang || project.targetLang || ""}`,
        entryCount: 0,
        termCount: 0,
    ...
    (pkg.resources?.terms || []).forEach((term) => {
      const resource = {
        ...
        languagePair:
          term.languagePair ||
          `${term.sourceLang || project.sourceLang || ""}::${term.targetLang || project.targetLang || ""}`,
        entryCount: 0,
        termCount: 0,
  ...
  async function saveProjectPackage(pkg) {
    ...
    const validationReportResult = await writeOptionalValidationReport(
      packagePath,
      packageWithMetadata.validation || {}
    );
    ...
    const nextManifest = defaultManifest({
      ...(manifest || {}),
      projects: upsertById(manifest?.projects || [], projectRef).sort(
        (a, b) => new Date(b.lastSavedAt || 0) - new Date(a.lastSavedAt || 0)
      ),
  ...
  async function buildHealthReport({ projects = [], tmEntries = [], terms = [], dirtyProjectIds = [] } = {}) {
    ...
    report.preserved.push(
      `${visibleProjects.length} visible project package${visibleProjects.length === 1 ? "" : "s"} in the workspace folder.`
    );
    report.preserved.push(
      `${manifest.projects.length} project package${manifest.projects.length === 1 ? "" : "s"} in the workspace manifest.`
    );
    report.preserved.push(
      `${visibleResources.length} visible resource reference${visibleResources.length === 1 ? "" : "s"} in the workspace folder.`
    );
    report.preserved.push(
      `${manifest.resources.length} resource reference${manifest.resources.length === 1 ? "" : "s"} in the workspace manifest.`
    );
    if (dirtyProjectIds.length) {
      report.warnings.push(
        `${dirtyProjectIds.length} open project package${dirtyProjectIds.length === 1 ? " has" : "s have"} unsaved folder changes.`
      );
    }
    ...
    for (const projectRef of manifest.projects || []) {
      try {
        ...
        report.errors.push(
          workspaceSafeLabel(error.message || error, "Project has an invalid workspace package path.")
        );
      }
    }
    ...
    projects.forEach((project) => {
      ...
      (project.resourceLinks || []).forEach((link) => {
        ...
        if (seenLinks.has(key))
          report.simplified.push(`${projectLabel} has a duplicate ${link.type} link named ${linkLabel}.`);
        seenLinks.add(key);
        const cacheHasResource = link.type === "tm" ? tmSet.has(key) : tbSet.has(key);
        if (!cacheHasResource && resourceSet.has(key)) {
          report.risky.push(
            `${projectLabel} links to ${linkLabel}, which is present in the workspace manifest but not loaded in the local cache.`
          );
