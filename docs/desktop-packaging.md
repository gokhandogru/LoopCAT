...
The GitHub Actions workflow in `.github/workflows/desktop-release.yml` builds the same desktop artifacts on Windows, macOS, and Linux when run manually or when a `v*` tag is pushed. Each platform job uploads only public download files plus its platform checksum file; unpacked app folders and builder debug sidecars are retained only inside the job workspace for verification. The final `release-bundle` job downloads all platform artifacts, runs `pnpm run verify:download-bundle -- --dist release-dist`, removes nested platform checksum sidecars while generating a combined `SHA256SUMS.txt`, verifies the combined file, and uploads the `LoopCAT-All-Platforms` artifact for publication.
