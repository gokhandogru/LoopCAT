...
Implementation state: substantial P0-P3 delivery, with target editing/autosave extracted and the remaining P1-08 coordinator work plus the performance stretch target still partial
...
- Packaged production JavaScript graph: 2,397,376 bytes across five modules, including both mutually exclusive startup paths.
- Hosted/desktop initial production `app.js`: 954,117 bytes minified and 264,161 bytes gzip. Its two locale chunks remain lazy.
- Direct-file web fallback `app-file.js`: 1,198,666 bytes minified and 347,126 bytes gzip; it is self-contained because browsers block module imports from `file://` and is not executed by HTTP(S) or Electron.
- Initial synchronous JavaScript is more than 40% below the recorded approximately 2.62 MB baseline. Hosted gzip remains within the 250 KiB-class budget but is 14,161 bytes above a strict 250,000-byte interpretation; the hosted entry remains 204,117 bytes above the 750 KB minified stretch target.
...
Continue P1-08 with a behavior-preserving segment-confirmation orchestration extraction. Target editing and autosave ownership are now complete, so the next safe boundary is the confirmation busy-state, validation, command, persistence, secondary TM/activity result, and confirm-and-next lifecycle rather than another state migration.

Entry criteria:

- Characterize confirm and confirm-and-next, missing protected-tag blocking, pending typing finalization, primary save and post-save rollback, secondary TM/activity failures, filter/progress updates, focus/navigation, Undo/Redo, reload recovery, and large-project behavior.
...
- Move segment-confirmation UI/busy/navigation orchestration behind a checked controller interface that receives EditorSessionStore, CommandBus, validation, repositories, TM/activity services, status, progress, and selection as injected boundaries; keep ordinary target editing, structural split/merge, review-state changes, and bulk import/pretranslation changes separate.
- Do not combine state ownership changes with visual redesign, storage migration, provider changes, or new features.

Exit criteria:

- The confirmation controller owns confirmation event, busy, focus, and confirm-and-next orchestration without owning domain records or bypassing CommandBus/EditorSessionStore.
- Ordinary confirmation behavior no longer depends on coordinator-private busy or event state, and release verification prevents those responsibilities from returning after extraction.
