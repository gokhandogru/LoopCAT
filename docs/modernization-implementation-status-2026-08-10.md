...
- Packaged production JavaScript graph: 2,386,344 bytes across five modules, including both mutually exclusive startup paths.
- Hosted/desktop initial production `app.js`: 948,601 bytes minified and 261,198 bytes gzip. Its two locale chunks remain lazy.
- Direct-file web fallback `app-file.js`: 1,193,150 bytes minified and 344,207 bytes gzip; it is self-contained because browsers block module imports from `file://` and is not executed by HTTP(S) or Electron.
- Initial synchronous JavaScript is more than 40% below the recorded approximately 2.62 MB baseline. Hosted gzip remains within the 250 KiB-class budget but is 11,198 bytes above a strict 250,000-byte interpretation; the hosted entry remains 198,601 bytes above the 750 KB minified stretch target.
...
The new checked boundaries are active, but `app.js` remains the compatibility coordinator and is still approximately 19,500 source lines, including the isolated workflow-test source section. Production strips the test driver, but the roadmap's source goal of a bootstrap under 300 lines is not met.
...
1. Replace remaining mutable compatibility state with injected repositories/controllers; the global module dependency bridge is complete, but the legacy coordinator state still mirrors the checked AppStore.
...
Continue P1-08 by making the checked AppStore authoritative for the first navigation/selection state slice instead of mirroring it in the legacy coordinator.

Entry criteria:

- Characterize `state.view`, `state.documentFilter`, `state.activeIndex`, `state.focusMode`, and the selected project/segment identifiers across Projects → dashboard → editor, project/file switching, Focus mode, filters, Undo/Redo, and reload recovery.
...
- Move one navigation/selection field family at a time behind AppStore selectors/actions; keep temporary read-through accessors for legacy render functions until every writer for that field is migrated.
- Do not combine state ownership changes with visual redesign, storage migration, provider changes, or new features.

Exit criteria:

- AppStore is the only writer for the selected navigation/selection fields, with checked selectors/actions and no new global mutable state.
- The migrated path no longer uses `syncLegacyApplicationState` for those fields, while unmigrated render callers continue through narrow read-only accessors.
