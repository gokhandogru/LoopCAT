...
- Packaged production JavaScript graph: 2,386,974 bytes across five modules, including both mutually exclusive startup paths.
- Hosted/desktop initial production `app.js`: 948,916 bytes minified and 262,235 bytes gzip. Its two locale chunks remain lazy.
- Direct-file web fallback `app-file.js`: 1,193,465 bytes minified and 345,081 bytes gzip; it is self-contained because browsers block module imports from `file://` and is not executed by HTTP(S) or Electron.
- Initial synchronous JavaScript is more than 40% below the recorded approximately 2.62 MB baseline. Hosted gzip remains within the 250 KiB-class budget but is 12,235 bytes above a strict 250,000-byte interpretation; the hosted entry remains 198,916 bytes above the 750 KB minified stretch target.
...
The new checked boundaries are active, but `app.js` remains the compatibility coordinator and is still 13,858 source lines. The 6,371-line workflow driver is now external and composed only into the test graph, but the roadmap's source goal of a bootstrap under 300 lines is not met.
...
1. Extract the remaining editor mutation orchestration from the compatibility coordinator now that route, selection, filters, project/session records, virtualization, commands, and persistence boundaries are explicit.
...
Continue P1-08 with a behavior-preserving target-editing and autosave orchestration extraction. EditorSessionStore ownership is now complete, so the next safe boundary is the UI-to-command/persistence lifecycle for ordinary target edits rather than another state migration.

Entry criteria:

- Characterize target typing/composition, copy-source and match/tag producers, edit-session coalescing, pending-save queues, retry/flush, selection/caret restoration, commands and Undo/Redo, reload recovery, typing latency, and large-project behavior.
...
- Move the ordinary target-edit event and autosave lifecycle behind checked controller/service interfaces that receive EditorSessionStore, CommandBus, repositories, status, and selection as injected boundaries; keep structural split/merge and bulk import/pretranslation changes separate.
- Do not combine state ownership changes with visual redesign, storage migration, provider changes, or new features.

Exit criteria:

- The target-edit controller owns DOM input/composition/caret orchestration, while the autosave service owns pending timers, retry, flush, and persistence sequencing; neither owns domain records or bypasses CommandBus/EditorSessionStore.
- Ordinary target-edit behavior no longer depends on coordinator-private event or timer state, and release verification prevents those responsibilities from returning after extraction.
