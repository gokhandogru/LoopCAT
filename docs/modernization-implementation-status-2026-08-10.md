...
- Packaged production JavaScript graph: 2,393,220 bytes across five modules, including both mutually exclusive startup paths.
- Hosted/desktop initial production `app.js`: 952,039 bytes minified and 262,657 bytes gzip. Its two locale chunks remain lazy.
- Direct-file web fallback `app-file.js`: 1,196,588 bytes minified and 345,612 bytes gzip; it is self-contained because browsers block module imports from `file://` and is not executed by HTTP(S) or Electron.
- Initial synchronous JavaScript is more than 40% below the recorded approximately 2.62 MB baseline. Hosted gzip remains within the 250 KiB-class budget but is 12,657 bytes above a strict 250,000-byte interpretation; the hosted entry remains 202,039 bytes above the 750 KB minified stretch target.
...
The new checked boundaries are active, but `app.js` remains the compatibility coordinator and is still 13,843 source lines. The 6,367-line workflow driver is now external and composed only into the test graph, but the roadmap's source goal of a bootstrap under 300 lines is not met.
...
1. Continue removing temporary EditorSessionStore compatibility accessors one family at a time. Project-summary, project-term, activity-event, QA-check, quality-risk, and AppStore navigation/selection ownership are complete; the progress-summary cache and core project/segment records remain.
...
Continue P1-08 with the final low-coupling derived EditorSessionStore field: the project progress-summary cache. Project-summary, project-term, activity-event, QA-check, quality-risk, and AppStore navigation/selection ownership are complete.

Entry criteria:

- Characterize progress-summary loading, replacement, project switching, dashboard rendering, analysis refresh, import/export, recovery, and reload behavior.
...
- Move `progressSummary` behind explicit EditorSessionStore selector/action APIs and injected analysis/dashboard calls; retain no compatibility setter for the migrated field.
- Do not combine state ownership changes with visual redesign, storage migration, provider changes, or new features.

Exit criteria:

- EditorSessionStore is the only writer for the progress-summary cache, with checked selector/action APIs and no new global mutable state.
- Neither migrated cache relies on `attachCompatibility`; unmigrated families continue through the narrowed bridge.
