...
Implementation state: substantial P0-P3 delivery, with target editing, autosave, and segment confirmation extracted and the remaining P1-08 coordinator work plus the performance stretch target still partial
...
- Packaged production JavaScript graph: 2,405,688 bytes across five modules, including both mutually exclusive startup paths.
- Hosted/desktop initial production `app.js`: 958,273 bytes minified and 265,359 bytes gzip. Its two locale chunks remain lazy.
- Direct-file web fallback `app-file.js`: 1,202,822 bytes minified and 348,280 bytes gzip; it is self-contained because browsers block module imports from `file://` and is not executed by HTTP(S) or Electron.
- Initial synchronous JavaScript is more than 40% below the recorded approximately 2.62 MB baseline. Hosted gzip remains within the 250 KiB-class budget but is 15,359 bytes above a strict 250,000-byte interpretation; the hosted entry remains 208,273 bytes above the 750 KB minified stretch target.
...
Continue P1-08 with a behavior-preserving discrete target-producer orchestration extraction. Target editing, autosave, and segment-confirmation ownership are now complete, so the next safe boundary is the shared Copy Source, TM match/concordance insertion, and protected-tag insertion command lifecycle rather than another state migration.

Entry criteria:

- Characterize Copy Source, TM match/concordance insertion, protected-tag insertion at collapsed and ranged selections, pending typing finalization, target history/provenance, filter/progress updates, caret/focus, autosave, failure recovery, Undo/Redo, reload recovery, and large-project behavior.
...
- Move the shared discrete target-producer command, mutation, rendering, persistence scheduling, selection, and failure-recovery orchestration behind a checked controller interface that receives EditorSessionStore, CommandBus, EditTarget finalization, AutosaveService, status, progress, history, and selection as injected boundaries; keep ordinary typing, confirmation, structural split/merge, review-state changes, and bulk import/pretranslation changes separate.
- Do not combine state ownership changes with visual redesign, storage migration, provider changes, or new features.

Exit criteria:

- The target-producer controller owns the shared Copy Source, TM/concordance target, and protected-tag command lifecycle without owning domain records or bypassing CommandBus/EditorSessionStore.
- Discrete producer behavior no longer depends on coordinator-private command, selection, rendering, or failure-recovery orchestration, and release verification prevents those responsibilities from returning after extraction.
