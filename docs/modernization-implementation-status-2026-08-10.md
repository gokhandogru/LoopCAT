...
- Packaged production JavaScript graph: 2,372,906 bytes across five modules, including both mutually exclusive startup paths.
- Hosted/desktop initial production `app.js`: 941,882 bytes minified and 261,254 bytes gzip. Its two locale chunks remain lazy.
- Direct-file web fallback `app-file.js`: 1,186,431 bytes minified and 344,276 bytes gzip; it is self-contained because browsers block module imports from `file://` and is not executed by HTTP(S) or Electron.
- Initial synchronous JavaScript is more than 40% below the recorded approximately 2.62 MB baseline. Hosted gzip remains within the 250 KiB-class budget but is 11,254 bytes above a strict 250,000-byte interpretation; the 750 KB minified stretch target is not yet met.
...
Continue P1-08 with the distinct Anthropic Messages provider behind an independently checked provider adapter.

Entry criteria:

- Preserve Anthropic's provider identifier, default URL/model, `/v1/models` and `/v1/messages` routes, `x-api-key` and `anthropic-version` headers, bounded `max_tokens`, content-block parsing, usage metadata, timeout/abort behavior, endpoint allowlist, consent/redaction rule, cancellation path, and local-first/offline behavior.
...
- Move Anthropic alone behind the existing registry/service interface before combining any other distinct provider protocols. Keep provider selection, administration UI, prompt policy, consent, key storage, jobs, commands, persistence, and status outside the adapter.
