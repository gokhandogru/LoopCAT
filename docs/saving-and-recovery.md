...
Open **Workspace** for the protection indicator, which distinguishes **local saving** from **external backup**. Routine Saving/Saved messages do not interrupt editing. Segment status describes translation progress; Workspace reports persistence and backups. A successful local save does not mean that a copy exists outside the browser profile. Save failures and conflicts remain visible until addressed. Unrelated activity or export notices do not clear those failures. **Retry local save** retries pending changes. **Emergency export** downloads current in-memory source and target text without requiring QA, indexes, or activity logging.
