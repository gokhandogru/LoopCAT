...
LoopCAT implements the core offline CAT workflow, recovery model, editor, reversible commands, and modular application architecture. Version `0.0.4-dev.20260923` is the current unsigned development preview, with [Arabic localization and right-to-left interface support](docs/releases/0.0.4-dev.20260923.md). The [current GitHub prerelease](https://github.com/gokhandogru/LoopCAT/releases/tag/preview-0.0.4-dev.20260923) provides web, Windows, macOS, and Linux packages built from one repository revision. The local [download manifest](downloads/release.json) identifies the matching Windows and web repository mirrors. This preview is not yet a signed or notarized production-qualified release.
...
| Repository downloads      | Version `0.0.4-dev.20260923` provides a current web bundle plus unsigned Windows installer and portable packages with SHA-256 checksums in [`downloads/`](downloads/).                                                                                                                                                                             |
...
Use the versioned application packages in the [current GitHub prerelease](https://github.com/gokhandogru/LoopCAT/releases/tag/preview-0.0.4-dev.20260923), or browse [all releases](https://github.com/gokhandogru/LoopCAT/releases). Do not use GitHub's automatic **Source code** ZIP or TAR.GZ as an application package. The repository's [`downloads/`](downloads/) directory mirrors the current web and Windows ZIP packages.

- Web: download [LoopCAT Web 0.0.4-dev.20260923.zip](https://github.com/gokhandogru/LoopCAT/releases/download/preview-0.0.4-dev.20260923/LoopCAT.Web.0.0.4-dev.20260923.zip), extract it, and open `index.html`. For installable offline PWA behavior, serve the extracted folder from a local or hosted HTTP/HTTPS origin; direct `file://` opening still runs the app, but browsers do not allow service-worker installation from local files.
- Windows installer: download `LoopCAT Setup 0.0.4-dev.20260923.exe` and run it. The repository mirror [`LoopCAT.Windows.Setup.0.0.4-dev.20260923.zip`](downloads/LoopCAT.Windows.Setup.0.0.4-dev.20260923.zip) contains the same installer.
- Windows portable: download `LoopCAT 0.0.4-dev.20260923.exe` and run it without installation. The repository mirror [`LoopCAT.0.0.4-dev.20260923.Portable.zip`](downloads/LoopCAT.0.0.4-dev.20260923.Portable.zip) contains the same executable.
- macOS (Apple Silicon / ARM64): download the [DMG](https://github.com/gokhandogru/LoopCAT/releases/download/preview-0.0.4-dev.20260923/LoopCAT-0.0.4-dev.20260923-arm64.dmg) for the normal drag-to-Applications flow, or the [macOS ZIP](https://github.com/gokhandogru/LoopCAT/releases/download/preview-0.0.4-dev.20260923/LoopCAT-0.0.4-dev.20260923-arm64-mac.zip) when you prefer a directly extracted app bundle.
- Linux (x64 / AMD64): download the [AppImage](https://github.com/gokhandogru/LoopCAT/releases/download/preview-0.0.4-dev.20260923/LoopCAT-0.0.4-dev.20260923.AppImage) for a portable launch, or the [DEB package](https://github.com/gokhandogru/LoopCAT/releases/download/preview-0.0.4-dev.20260923/loopcat_0.0.4-dev.20260923_amd64.deb) and install it with `sudo apt install ./<downloaded-file>.deb`.
- Checksums: download the release asset `SHA256SUMS.txt` and compare the full SHA-256 value for the exact filename you downloaded. The repository mirror also provides [`LoopCAT.0.0.4-dev.20260923.SHA256SUMS.txt`](downloads/LoopCAT.0.0.4-dev.20260923.SHA256SUMS.txt) for its three ZIPs.
...
- Choose the LoopCAT interface language from the Workspace menu, including built-in Arabic, Catalan, English, and Turkish, import custom UI translation JSON, and export the English UI source catalog for translators.
...
- Version `0.0.4-dev.20260923` provides web, Windows installer/portable, macOS DMG/ZIP, and Linux AppImage/DEB packages on the GitHub prerelease. A stable all-platform release still requires signing or notarization where applicable and the outstanding clean-machine qualification matrix.
...
Target TXT, DOCX, XLIFF, and Other formats delivery exports are blocked when no segments are available, protected inline tags or placeholders are missing from authored targets, original structure metadata or required reconstruction maps such as package item indexes or subtitle cue timing are missing, malformed reconstruction indexes are present, unsafe HTML including entity-obfuscated or CSS-escaped scriptable attributes is present, XML-invalid output would be produced, or forbidden target terminology appears. Incomplete exports require confirmation: monolingual delivery formats use source text for whitespace-only empty targets, while bilingual interchange formats such as XLIFF, PO/POT, TTX/TXML, Qt TS, and delimited files with a target column preserve meaningful empty targets and untranslated states. Non-empty draft targets export unchanged and are reported in the warning. Export-time fallbacks never modify editor targets, status, revision history, storage, or translation memory. DOCX and localization exports obey the selected document and show a visible warning instead of silently switching to another file type. Target TXT and generic XLIFF exports also obey the selected document when one is selected; choosing All documents exports the whole project. Bilingual review exports remain available for review workflows and can document empty or untranslated projects, but XML-invalid characters are still blocked so XML-backed delivery files are not corrupted.
...
Before packaging a release, run `pnpm run verify:provenance -- --allow-untagged`, `pnpm install --frozen-lockfile`, `pnpm run verify:release`, then run the browser tests and a small manual import/edit/export smoke test. The provenance gate fails before packaging if Git is not available, Git metadata such as `.git/HEAD` is missing, the checkout is dirty, or the checked-out commit does not match the release tag or CI SHA. The browser runner includes a large-project fixture so release checks cover academic-length package, autosave, QA, and backup behavior. The desktop release workflow produces platform-specific artifacts, verifies the combined Windows/macOS/Linux bundle, builds the static web ZIP separately, and generates checksum files for publication.
