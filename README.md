...
- Linux x64 installer (recommended for Ubuntu/Debian): download [`loopcat_0.0.4-dev.20260913_amd64.deb`](https://github.com/gokhandogru/LoopCAT/releases/download/preview-0.0.4-dev.20260913/loopcat_0.0.4-dev.20260913_amd64.deb), then run `sudo apt install ./loopcat_0.0.4-dev.20260913_amd64.deb` from its directory. An AppImage alternative, Linux-specific checksums, and [installation/verification details](docs/releases/0.0.4-dev.20260913-linux.md) are also available. Tested on Ubuntu 24.04 x64; these remain development previews.
...
- Version `0.0.4-dev.20260913` provides the repository web bundle, unsigned Windows installer/portable downloads, and Linux x64 DEB/AppImage assets on the GitHub preview release. Linux installation and launch evidence, separate checksums, and portability limits are documented in the [Linux release supplement](docs/releases/0.0.4-dev.20260913-linux.md). A formal all-platform release still requires signing/notarization where applicable and the outstanding qualification matrix.
