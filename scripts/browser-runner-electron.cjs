...
const productionRendererRoot = path.join(root, ".cache", "renderer", "production");
const testRendererRoot = path.join(root, ".cache", "renderer", "test");
...
for (const asset of JSON.parse(
  require("node:fs").readFileSync(path.join(productionRendererRoot, "assets.json"), "utf8")
)) {
  generatedFiles.set(asset, path.join(productionRendererRoot, asset));
}
for (const asset of JSON.parse(require("node:fs").readFileSync(path.join(testRendererRoot, "assets.json"), "utf8"))) {
  generatedFiles.set(`renderer-test/${asset}`, path.join(testRendererRoot, asset));
}
...
app
  ...
  .then(async () => {
    ...
    const requestedTest = String(process.env.LOOPCAT_BROWSER_TEST_FILTER || "").trim();
    const requestedTimeout = Number(process.env.LOOPCAT_BROWSER_TEST_CASE_TIMEOUT_MS || 0);
    ...
    windowRef.webContents.on("did-fail-load", (_event, errorCode, errorDescription, validatedUrl, isMainFrame) => {
      ...
      send(
        `LoopCAT Browser Test Runner - load failure${isMainFrame ? "" : " in subframe"}: ${errorDescription} (${errorCode}) ${validatedUrl}`
      );
    });
    ...
    setTimeout(async () => {
      ...
      finish(
        1,
        `Browser runner timed out after ${timeoutMs} ms. Last title: ${lastTitle || "(none)"}${diagnostic ? `\n${diagnostic}` : ""}`
      );
    }, timeoutMs).unref();
  })
  .catch((error) => {
