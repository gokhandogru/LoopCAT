...
function togetherStatusError(data, status, model = "") {
  const raw = redactSensitiveText(data?.error?.message || data?.message || data?.error || "").trim();
  if (status === 401 || status === 403) return "Together AI rejected the request. Add or check the Together AI API key.";
  if ((status === 404 || /model/i.test(raw)) && model) return `Model ${model} was not found by Together AI.`;
  return raw || `Together AI request failed with status ${status}.`;
}

async function togetherJson(endpoint, options = {}, config = {}) {
  const url = togetherApiUrl(config.baseUrl || TOGETHER_DEFAULT_BASE_URL, endpoint);
  let result = null;
  try {
    result = await fetchJsonWithTimeout(url, options, config);
  } catch (error) {
    if (String(error?.message || "").includes("canceled") || String(error?.message || "").includes("timed out")) throw error;
    throw new Error(`Together AI is not reachable at ${normalizeTogetherBaseUrl(config.baseUrl || TOGETHER_DEFAULT_BASE_URL)}.`);
  }
  if (!result.response?.ok) {
    throw new Error(togetherStatusError(result.data, result.response?.status, config.model));
  }
  return result.data;
}

function extractTogetherResponseText(data) {
  const messageContent = data?.choices?.[0]?.message?.content;
  if (typeof messageContent === "string") return messageContent;
  if (Array.isArray(messageContent)) {
    return messageContent.map((part) => {
...
const TogetherProvider = {
  id: "together",
  name: "Together AI",
  defaultBaseUrl: TOGETHER_DEFAULT_BASE_URL,
  defaultModel: TOGETHER_DEFAULT_MODEL,
  async testConnection(config = {}) {
    if (!String(config.apiKey || "").trim()) throw new Error(togetherProviderAuthError());
    const baseUrl = normalizeTogetherBaseUrl(config.baseUrl || TOGETHER_DEFAULT_BASE_URL);
    const data = await togetherJson("/models", { method: "GET", headers: bearerAuthHeaders(config) }, { ...config, baseUrl });
    return {
      ok: true,
  ...
  async listModels(config = {}) {
    ...
    const models = records.map((model) => {
      ...
      return {
        name: String(model.id || model.name || "").trim(),
        size: model.size || 0,
        modifiedAt: Number.isFinite(created) ? new Date(created * 1000).toISOString() : (model.updated_at || model.created_at || "")
      };
    }).filter((model) => model.name);
    return { models, raw: data };
  },
  async translateSegment(config = {}, request = {}) {
    const settings = defaultLocalAiSettings({ ...config, providerId: "together", model: config.model || request.model }, request.project);
    const baseUrl = normalizeTogetherBaseUrl(config.baseUrl || settings.baseUrl || TOGETHER_DEFAULT_BASE_URL);
    const model = String(config.model || request.model || settings.model || TOGETHER_DEFAULT_MODEL).trim() || TOGETHER_DEFAULT_MODEL;
    const sourceText = String(request.text ?? request.segment?.source ?? "");
    if (!sourceText.trim()) throw new Error("The segment has no source text.");
    if (!String(config.apiKey || "").trim()) throw new Error(togetherProviderAuthError());
    const prompt = request.prompt || buildTranslateGemmaPrompt({
    ...
    const startedAt = localAiStartedAt();
    const data = await togetherJson("/chat/completions", {
      method: "POST",
      headers: bearerAuthHeaders(config, { "Content-Type": "application/json" }),
      body: JSON.stringify({
        model,
    ...
    const rawOutput = extractTogetherResponseText(data);
    if (typeof rawOutput !== "string") throw new Error("Together AI returned a malformed chat response.");
    const translatedText = cleanModelTranslationOutput(rawOutput, sourceText);
    if (!translatedText.trim()) throw new Error("Together AI returned an empty translation for this segment.");
    return {
      translatedText,
      rawOutput,
      provider: "Together AI",
...
function openRouterProviderAuthError() {
  return "Add an OpenRouter API key before using OpenRouter for pre-translation.";
}

function openRouterStatusError(data, status, model = "") {
  const raw = redactSensitiveText(data?.error?.message || data?.message || data?.error || "").trim();
  if (status === 401 || status === 403) return "OpenRouter rejected the request. Add or check the OpenRouter API key.";
  if (status === 402) return "OpenRouter reported insufficient credits for this request.";
  if ((status === 404 || /model/i.test(raw)) && model) return `Model ${model} was not found by OpenRouter.`;
  return raw || `OpenRouter request failed with status ${status}.`;
}

async function openRouterJson(endpoint, options = {}, config = {}) {
  const url = openRouterApiUrl(config.baseUrl || OPENROUTER_DEFAULT_BASE_URL, endpoint);
  let result = null;
  try {
    result = await fetchJsonWithTimeout(url, options, config);
  } catch (error) {
    if (String(error?.message || "").includes("canceled") || String(error?.message || "").includes("timed out")) throw error;
    throw new Error(`OpenRouter is not reachable at ${normalizeOpenRouterBaseUrl(config.baseUrl || OPENROUTER_DEFAULT_BASE_URL)}.`);
  }
  if (!result.response?.ok) {
    throw new Error(openRouterStatusError(result.data, result.response?.status, config.model));
  }
  return result.data;
}
...
const OpenRouterProvider = {
  ...
  async testConnection(config = {}) {
    if (!String(config.apiKey || "").trim()) throw new Error(openRouterProviderAuthError());
    const baseUrl = normalizeOpenRouterBaseUrl(config.baseUrl || OPENROUTER_DEFAULT_BASE_URL);
    const data = await openRouterJson("/models", { method: "GET", headers: bearerAuthHeaders(config) }, { ...config, baseUrl });
    return {
      ok: true,
      provider: "OpenRouter",
      baseUrl,
  ...
  async listModels(config = {}) {
    ...
    const models = Array.isArray(data?.data)
      ? data.data.map((model) => {
        const created = Number(model.created);
        const updatedAt = model.updated_at || model.updatedAt || "";
        return {
          name: String(model.id || model.name || "").trim(),
          size: Number(model.context_length || model.contextLength || 0) || model.size || 0,
          modifiedAt: Number.isFinite(created) ? new Date(created * 1000).toISOString() : updatedAt
        };
      }).filter((model) => model.name)
      : [];
    return { models, raw: data };
  ...
  async translateSegment(config = {}, request = {}) {
    const settings = defaultLocalAiSettings({ ...config, providerId: "openrouter", model: config.model || request.model }, request.project);
    const baseUrl = normalizeOpenRouterBaseUrl(config.baseUrl || settings.baseUrl || OPENROUTER_DEFAULT_BASE_URL);
    const model = String(config.model || request.model || settings.model || OPENROUTER_DEFAULT_MODEL).trim() || OPENROUTER_DEFAULT_MODEL;
    const sourceText = String(request.text ?? request.segment?.source ?? "");
    if (!sourceText.trim()) throw new Error("The segment has no source text.");
    if (!String(config.apiKey || "").trim()) throw new Error(openRouterProviderAuthError());
    const prompt = request.prompt || buildTranslateGemmaPrompt({
    ...
    const rawOutput = extractOpenRouterResponseText(data);
    if (typeof rawOutput !== "string") throw new Error("OpenRouter returned a malformed chat response.");
    const translatedText = cleanModelTranslationOutput(rawOutput, sourceText);
    if (!translatedText.trim()) throw new Error("OpenRouter returned an empty translation for this segment.");
    return {
      translatedText,
      rawOutput,
      provider: "OpenRouter",
...
function huggingFaceProviderAuthError() {
  return "Add a Hugging Face token before using Hugging Face Inference Providers for pre-translation.";
}

function huggingFaceStatusError(data, status, model = "") {
  const raw = redactSensitiveText(data?.error?.message || data?.error || data?.message || "").trim();
  if (status === 401 || status === 403) return "Hugging Face rejected the request. Add or check the Hugging Face token.";
  if (status === 402) return "Hugging Face reported insufficient credits or quota for this request.";
  if (status === 429) return "Hugging Face rate-limited this request. Wait a moment or choose another provider/model.";
  if ((status === 404 || /model/i.test(raw)) && model) return `Model ${model} was not found by Hugging Face Inference Providers.`;
  return raw || `Hugging Face request failed with status ${status}.`;
}

async function huggingFaceJson(endpoint, options = {}, config = {}) {
  const url = huggingFaceApiUrl(config.baseUrl || HUGGINGFACE_DEFAULT_BASE_URL, endpoint);
  let result = null;
  try {
    result = await fetchJsonWithTimeout(url, options, config);
  } catch (error) {
    if (String(error?.message || "").includes("canceled") || String(error?.message || "").includes("timed out")) throw error;
    throw new Error(`Hugging Face Inference Providers is not reachable at ${normalizeHuggingFaceBaseUrl(config.baseUrl || HUGGINGFACE_DEFAULT_BASE_URL)}.`);
  }
  if (!result.response?.ok) {
    throw new Error(huggingFaceStatusError(result.data, result.response?.status, config.model));
  }
  return result.data;
}

function extractHuggingFaceResponseText(data) {
  const messageContent = data?.choices?.[0]?.message?.content;
  if (typeof messageContent === "string") return messageContent;
  if (Array.isArray(messageContent)) {
    return messageContent.map((part) => {
...
const HuggingFaceProvider = {
  ...
  defaultBaseUrl: HUGGINGFACE_DEFAULT_BASE_URL,
  defaultModel: HUGGINGFACE_DEFAULT_MODEL,
  async testConnection(config = {}) {
    if (!String(config.apiKey || "").trim()) throw new Error(huggingFaceProviderAuthError());
    const baseUrl = normalizeHuggingFaceBaseUrl(config.baseUrl || HUGGINGFACE_DEFAULT_BASE_URL);
    const data = await huggingFaceJson("/models", { method: "GET", headers: bearerAuthHeaders(config) }, { ...config, baseUrl });
    return {
      ok: true,
  ...
  async translateSegment(config = {}, request = {}) {
    const settings = defaultLocalAiSettings({ ...config, providerId: "huggingface", model: config.model || request.model }, request.project);
    const baseUrl = normalizeHuggingFaceBaseUrl(config.baseUrl || settings.baseUrl || HUGGINGFACE_DEFAULT_BASE_URL);
    const model = String(config.model || request.model || settings.model || HUGGINGFACE_DEFAULT_MODEL).trim() || HUGGINGFACE_DEFAULT_MODEL;
    const sourceText = String(request.text ?? request.segment?.source ?? "");
    if (!sourceText.trim()) throw new Error("The segment has no source text.");
    if (!String(config.apiKey || "").trim()) throw new Error(huggingFaceProviderAuthError());
    const prompt = request.prompt || buildTranslateGemmaPrompt({
    ...
    const startedAt = localAiStartedAt();
    const data = await huggingFaceJson("/chat/completions", {
      method: "POST",
      headers: bearerAuthHeaders(config, { "Content-Type": "application/json" }),
      body: JSON.stringify({
        model,
    ...
    const rawOutput = extractHuggingFaceResponseText(data);
    if (typeof rawOutput !== "string") throw new Error("Hugging Face returned a malformed chat response.");
    const translatedText = cleanModelTranslationOutput(rawOutput, sourceText);
    if (!translatedText.trim()) throw new Error("Hugging Face returned an empty translation for this segment.");
    return {
      translatedText,
      rawOutput,
      provider: "Hugging Face Inference Providers",
...
function deepInfraProviderAuthError() {
  return "Add a DeepInfra API key before using DeepInfra for pre-translation.";
}

function deepInfraStatusError(data, status, model = "") {
  const raw = redactSensitiveText(data?.error?.message || data?.message || data?.error || "").trim();
  if (status === 401 || status === 403) return "DeepInfra rejected the request. Add or check the DeepInfra API key.";
  if ((status === 404 || /model/i.test(raw)) && model) return `Model ${model} was not found by DeepInfra.`;
  return raw || `DeepInfra request failed with status ${status}.`;
}

async function deepInfraJson(endpoint, options = {}, config = {}) {
  const url = deepInfraApiUrl(config.baseUrl || DEEPINFRA_DEFAULT_BASE_URL, endpoint);
  let result = null;
  try {
    result = await fetchJsonWithTimeout(url, options, config);
  } catch (error) {
    if (String(error?.message || "").includes("canceled") || String(error?.message || "").includes("timed out")) throw error;
    throw new Error(`DeepInfra is not reachable at ${normalizeDeepInfraBaseUrl(config.baseUrl || DEEPINFRA_DEFAULT_BASE_URL)}.`);
  }
  if (!result.response?.ok) {
    throw new Error(deepInfraStatusError(result.data, result.response?.status, config.model));
  }
  return result.data;
}

function extractDeepInfraResponseText(data) {
  const messageContent = data?.choices?.[0]?.message?.content;
  if (typeof messageContent === "string") return messageContent;
  if (Array.isArray(messageContent)) {
    return messageContent.map((part) => {
...
const DeepInfraProvider = {
  ...
  defaultModel: DEEPINFRA_DEFAULT_MODEL,
  async testConnection(config = {}) {
    if (!String(config.apiKey || "").trim()) throw new Error(deepInfraProviderAuthError());
    const baseUrl = normalizeDeepInfraBaseUrl(config.baseUrl || DEEPINFRA_DEFAULT_BASE_URL);
    const data = await deepInfraJson("/models", { method: "GET", headers: bearerAuthHeaders(config) }, { ...config, baseUrl });
    return {
      ok: true,
      provider: "DeepInfra",
      baseUrl,
  ...
  async translateSegment(config = {}, request = {}) {
    const settings = defaultLocalAiSettings({ ...config, providerId: "deepinfra", model: config.model || request.model }, request.project);
    const baseUrl = normalizeDeepInfraBaseUrl(config.baseUrl || settings.baseUrl || DEEPINFRA_DEFAULT_BASE_URL);
    const model = String(config.model || request.model || settings.model || DEEPINFRA_DEFAULT_MODEL).trim() || DEEPINFRA_DEFAULT_MODEL;
    const sourceText = String(request.text ?? request.segment?.source ?? "");
    if (!sourceText.trim()) throw new Error("The segment has no source text.");
    if (!String(config.apiKey || "").trim()) throw new Error(deepInfraProviderAuthError());
    const prompt = request.prompt || buildTranslateGemmaPrompt({
    ...
    const startedAt = localAiStartedAt();
    const data = await deepInfraJson("/chat/completions", {
      method: "POST",
      headers: bearerAuthHeaders(config, { "Content-Type": "application/json" }),
      body: JSON.stringify({
        model,
    ...
    const rawOutput = extractDeepInfraResponseText(data);
    if (typeof rawOutput !== "string") throw new Error("DeepInfra returned a malformed chat response.");
    const translatedText = cleanModelTranslationOutput(rawOutput, sourceText);
    if (!translatedText.trim()) throw new Error("DeepInfra returned an empty translation for this segment.");
    return {
      translatedText,
      rawOutput,
      provider: "DeepInfra",
...
function fireworksProviderAuthError() {
  return "Add a Fireworks AI API key before using Fireworks AI for pre-translation.";
}

function fireworksStatusError(data, status, model = "") {
  const raw = redactSensitiveText(data?.error?.message || data?.message || data?.error || "").trim();
  if (status === 401 || status === 403) return "Fireworks AI rejected the request. Add or check the Fireworks AI API key.";
  if ((status === 404 || /model/i.test(raw)) && model) return `Model ${model} was not found by Fireworks AI.`;
  return raw || `Fireworks AI request failed with status ${status}.`;
}

async function fireworksJson(endpoint, options = {}, config = {}) {
  const url = fireworksApiUrl(config.baseUrl || FIREWORKS_DEFAULT_BASE_URL, endpoint);
  let result = null;
  try {
    result = await fetchJsonWithTimeout(url, options, config);
  } catch (error) {
    if (String(error?.message || "").includes("canceled") || String(error?.message || "").includes("timed out")) throw error;
    throw new Error(`Fireworks AI is not reachable at ${normalizeFireworksBaseUrl(config.baseUrl || FIREWORKS_DEFAULT_BASE_URL)}.`);
  }
  if (!result.response?.ok) {
    throw new Error(fireworksStatusError(result.data, result.response?.status, config.model));
  }
  return result.data;
}

function extractFireworksResponseText(data) {
  const messageContent = data?.choices?.[0]?.message?.content;
  if (typeof messageContent === "string") return messageContent;
  if (Array.isArray(messageContent)) {
    return messageContent.map((part) => {
...
const FireworksProvider = {
  id: "fireworks",
  name: "Fireworks AI",
  defaultBaseUrl: FIREWORKS_DEFAULT_BASE_URL,
  defaultModel: FIREWORKS_DEFAULT_MODEL,
  async testConnection(config = {}) {
    if (!String(config.apiKey || "").trim()) throw new Error(fireworksProviderAuthError());
    const baseUrl = normalizeFireworksBaseUrl(config.baseUrl || FIREWORKS_DEFAULT_BASE_URL);
    const data = await fireworksJson("/models", { method: "GET", headers: bearerAuthHeaders(config) }, { ...config, baseUrl });
    return {
      ok: true,
  ...
  async translateSegment(config = {}, request = {}) {
    const settings = defaultLocalAiSettings({ ...config, providerId: "fireworks", model: config.model || request.model }, request.project);
    const baseUrl = normalizeFireworksBaseUrl(config.baseUrl || settings.baseUrl || FIREWORKS_DEFAULT_BASE_URL);
    const model = String(config.model || request.model || settings.model || FIREWORKS_DEFAULT_MODEL).trim() || FIREWORKS_DEFAULT_MODEL;
    const sourceText = String(request.text ?? request.segment?.source ?? "");
    if (!sourceText.trim()) throw new Error("The segment has no source text.");
    if (!String(config.apiKey || "").trim()) throw new Error(fireworksProviderAuthError());
    const prompt = request.prompt || buildTranslateGemmaPrompt({
    ...
    const startedAt = localAiStartedAt();
    const data = await fireworksJson("/chat/completions", {
      method: "POST",
      headers: bearerAuthHeaders(config, { "Content-Type": "application/json" }),
      body: JSON.stringify({
        model,
    ...
    const rawOutput = extractFireworksResponseText(data);
    if (typeof rawOutput !== "string") throw new Error("Fireworks AI returned a malformed chat response.");
    const translatedText = cleanModelTranslationOutput(rawOutput, sourceText);
    if (!translatedText.trim()) throw new Error("Fireworks AI returned an empty translation for this segment.");
    return {
      translatedText,
      rawOutput,
      provider: "Fireworks AI",
...
TogetherProvider.completePrompt = async function completePrompt(config = {}, request = {}) {
  const settings = defaultLocalAiSettings({ ...config, providerId: "together", model: config.model || request.model }, request.project);
  const baseUrl = normalizeTogetherBaseUrl(config.baseUrl || settings.baseUrl || TOGETHER_DEFAULT_BASE_URL);
  const model = String(config.model || request.model || settings.model || TOGETHER_DEFAULT_MODEL).trim() || TOGETHER_DEFAULT_MODEL;
  if (!String(config.apiKey || "").trim()) throw new Error(togetherProviderAuthError());
  ...
  return genericPromptResult("Together AI", "together", model, prompt, extractTogetherResponseText(data), startedAt, {
    promptTokens: data?.usage?.prompt_tokens || 0,
    completionTokens: data?.usage?.completion_tokens || 0,
    totalTokens: data?.usage?.total_tokens || 0
  });
...
OpenRouterProvider.completePrompt = async function completePrompt(config = {}, request = {}) {
  const settings = defaultLocalAiSettings({ ...config, providerId: "openrouter", model: config.model || request.model }, request.project);
  const baseUrl = normalizeOpenRouterBaseUrl(config.baseUrl || settings.baseUrl || OPENROUTER_DEFAULT_BASE_URL);
  const model = String(config.model || request.model || settings.model || OPENROUTER_DEFAULT_MODEL).trim() || OPENROUTER_DEFAULT_MODEL;
  if (!String(config.apiKey || "").trim()) throw new Error(openRouterProviderAuthError());
  ...
  return genericPromptResult("OpenRouter", "openrouter", model, prompt, extractOpenRouterResponseText(data), startedAt, {
    promptTokens: data?.usage?.prompt_tokens || 0,
    completionTokens: data?.usage?.completion_tokens || 0,
    totalTokens: data?.usage?.total_tokens || 0
  });
...
HuggingFaceProvider.completePrompt = async function completePrompt(config = {}, request = {}) {
  const settings = defaultLocalAiSettings({ ...config, providerId: "huggingface", model: config.model || request.model }, request.project);
  const baseUrl = normalizeHuggingFaceBaseUrl(config.baseUrl || settings.baseUrl || HUGGINGFACE_DEFAULT_BASE_URL);
  const model = String(config.model || request.model || settings.model || HUGGINGFACE_DEFAULT_MODEL).trim() || HUGGINGFACE_DEFAULT_MODEL;
  if (!String(config.apiKey || "").trim()) throw new Error(huggingFaceProviderAuthError());
  ...
  return genericPromptResult("Hugging Face Inference Providers", "huggingface", model, prompt, extractHuggingFaceResponseText(data), startedAt, {
    promptTokens: data?.usage?.prompt_tokens || 0,
    completionTokens: data?.usage?.completion_tokens || 0,
    totalTokens: data?.usage?.total_tokens || 0
  });
...
DeepInfraProvider.completePrompt = async function completePrompt(config = {}, request = {}) {
  const settings = defaultLocalAiSettings({ ...config, providerId: "deepinfra", model: config.model || request.model }, request.project);
  const baseUrl = normalizeDeepInfraBaseUrl(config.baseUrl || settings.baseUrl || DEEPINFRA_DEFAULT_BASE_URL);
  const model = String(config.model || request.model || settings.model || DEEPINFRA_DEFAULT_MODEL).trim() || DEEPINFRA_DEFAULT_MODEL;
  if (!String(config.apiKey || "").trim()) throw new Error(deepInfraProviderAuthError());
  ...
  return genericPromptResult("DeepInfra", "deepinfra", model, prompt, extractDeepInfraResponseText(data), startedAt, {
    promptTokens: data?.usage?.prompt_tokens || 0,
    completionTokens: data?.usage?.completion_tokens || 0,
    totalTokens: data?.usage?.total_tokens || 0
  });
...
FireworksProvider.completePrompt = async function completePrompt(config = {}, request = {}) {
  const settings = defaultLocalAiSettings({ ...config, providerId: "fireworks", model: config.model || request.model }, request.project);
  const baseUrl = normalizeFireworksBaseUrl(config.baseUrl || settings.baseUrl || FIREWORKS_DEFAULT_BASE_URL);
  const model = String(config.model || request.model || settings.model || FIREWORKS_DEFAULT_MODEL).trim() || FIREWORKS_DEFAULT_MODEL;
  if (!String(config.apiKey || "").trim()) throw new Error(fireworksProviderAuthError());
  ...
  return genericPromptResult("Fireworks AI", "fireworks", model, prompt, extractFireworksResponseText(data), startedAt, {
    promptTokens: data?.usage?.prompt_tokens || 0,
    completionTokens: data?.usage?.completion_tokens || 0,
    totalTokens: data?.usage?.total_tokens || 0
  });
