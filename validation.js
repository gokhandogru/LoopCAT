...
const PORTABLE_RECORD_ID_KEYS = new Set(["id", "projectId", "documentId", "segmentId", "workspaceId", "ownerId", "createdBy"]);
const VOID_TAGS = new Set(["area", "base", "br", "col", "embed", "hr", "img", "input", "link", "meta", "param", "source", "track", "wbr"]);
const UNSAFE_HTML_TAGS = new Set(["script", "iframe", "object", "embed", "link", "meta", "base", "form", "svg", "math"]);
...
const RECONSTRUCTION_SEGMENT_REQUIREMENTS = {
  ...
  strings: {
    label: "iOS strings",
    issue: "line mapping data",
    valid: (segment) => structureFormat(segment) === "apple-strings" && hasStructureNumber(segment?.structure?.lineIndex)
  },
  idml: {
    label: "IDML",
    issue: "story mapping data",
    valid: (segment) => structureFormat(segment) === "idml" &&
...
const OPENXML_TYPES = ["docm", "dotx", "dotm", "xlsx", "xlsm", "xltx", "xltm", "pptx", "pptm", "ppsx", "ppsm", "potx", "potm"];
OPENXML_TYPES.forEach((type) => {
  RECONSTRUCTION_SEGMENT_REQUIREMENTS[type] = {
    label: "OpenXML",
    issue: "package text mapping data",
    valid: (segment) => structureFormat(segment) === "openxml" &&
      String(segment?.structure?.path || "").trim() &&
...
OPENDOCUMENT_TYPES.forEach((type) => {
  RECONSTRUCTION_SEGMENT_REQUIREMENTS[type] = {
    ...
    valid: (segment) => structureFormat(segment) === "opendocument" &&
      String(segment?.structure?.path || "").trim() &&
      hasStructureNumber(segment?.structure?.itemIndex)
  };
});
...
["xhtml", "dita", "xini", "wix"].forEach((type) => {
  RECONSTRUCTION_SEGMENT_REQUIREMENTS[type] = {
    ...
    valid: (segment) => structureFormat(segment) === "generic-xml" && hasStructureNumber(segment?.structure?.itemIndex)
  };
});
...
["txml", "ttx"].forEach((type) => {
  RECONSTRUCTION_SEGMENT_REQUIREMENTS[type] = {
    ...
    valid: (segment) => structureFormat(segment) === "bilingual-xml" && hasStructureNumber(segment?.structure?.pairIndex)
  };
});
...
Object.assign(RECONSTRUCTION_SEGMENT_REQUIREMENTS, {
  ...
  php: {
    ...
    valid: (segment) => structureFormat(segment) === "code-string" && hasStructureNumber(segment?.structure?.tokenIndex)
  },
  ts: {
    label: "TS",
  ...
  dtd: {
    ...
    valid: (segment) => structureFormat(segment) === "quoted-text" && hasStructureNumber(segment?.structure?.tokenIndex)
  },
  mif: {
    label: "MIF",
...
function validateProjectResourceLinks(report, project, label = "Project", missingBucket = "errors", issueBucket = "errors") {
  ...
  project.resourceLinks.forEach((link, index) => {
    ...
    if (link.id !== undefined) {
      if (typeof link.id !== "string" || !link.id.trim()) {
        ...
        if (linkIds.has(link.id)) add(report, issueBucket, `Duplicate resource link ID in project manifest: ${link.id}.`);
        linkIds.add(link.id);
      }
    } else {
    ...
    if (RESOURCE_LINK_TYPES.has(type) && name) {
      ...
      if (linkKeys.has(linkKey)) add(report, issueBucket, `Duplicate resource link in project manifest: ${type}/${name}.`);
      linkKeys.add(linkKey);
    }
    if (type === "tm" && link.role === "main") mainTmLinks += 1;
  });
  if (mainTmLinks > 1) add(report, issueBucket, `${label} resource links contain multiple main translation memories.`);
}
...
function detectSecretFields(value, path = "") {
  ...
  Object.entries(value).forEach(([key, item]) => {
    ...
    if (key !== "apiKeyMode" && SECRET_FIELD_PATTERN.test(key) && String(item || "").trim() && !isSourceJsonPath(nextPath)) findings.push(nextPath);
...
function detectInvalidAiKeyModes(value, path = "") {
  ...
  Object.entries(value).forEach(([key, item]) => {
    const nextPath = path ? `${path}.${key}` : key;
    if (key === "apiKeyMode" && item !== undefined && item !== null && !AI_API_KEY_MODES.has(String(item || "").trim())) {
  ...
  if (isAiSuggestionLike(value) && !isSourceJsonPath(path)) {
    ...
    if (Array.isArray(value.explanation)) {
      value.explanation.forEach((item, index) => {
        if (hasSensitiveTextValue(item)) findings.push(path ? `${path}.explanation.${index}` : `explanation.${index}`);
      });
    }
  }
  Object.entries(value).forEach(([key, item]) => {
...
function detectSensitiveRecordIds(value, path = "") {
  ...
  Object.entries(value).forEach(([key, item]) => {
    const nextPath = path ? `${path}.${key}` : key;
    if (PORTABLE_RECORD_ID_KEYS.has(key) && hasSensitiveTextValue(item) && !isSourceJsonPath(nextPath)) findings.push(nextPath);
...
function detectProviderTraceFields(value, path = "") {
  ...
  Object.entries(value).forEach(([key, item]) => {
    const nextPath = path ? `${path}.${key}` : key;
    const hasPortableValue = item !== undefined && item !== null && (typeof item !== "string" || Boolean(item.trim()));
    if (hasPortableValue && PROVIDER_TRACE_FIELD_PATTERN.test(key) && !isSourceJsonPath(nextPath)) findings.push(nextPath);
...
function addSecretFindings(report, value, label) {
  ...
  if (sensitiveAiStyleGuides.length) {
    add(report, "errors", `${label} must not include credential-looking AI style instructions: ${sensitiveAiStyleGuides.join(", ")}.`);
  }
  const sensitiveAiSettingsMetadata = detectSensitiveAiSettingsMetadata(value);
  if (sensitiveAiSettingsMetadata.length) {
    add(report, "errors", `${label} must not include credential-looking AI settings metadata: ${sensitiveAiSettingsMetadata.join(", ")}.`);
  }
  const sensitiveActivityMetadata = detectSensitiveActivityMetadata(value);
  if (sensitiveActivityMetadata.length) {
    add(report, "errors", `${label} must not include credential-looking activity metadata: ${sensitiveActivityMetadata.join(", ")}.`);
  }
  const sensitiveProjectDomains = detectSensitiveProjectDomains(value);
  if (sensitiveProjectDomains.length) {
    add(report, "errors", `${label} must not include credential-looking project domain metadata: ${sensitiveProjectDomains.join(", ")}.`);
  }
  const sensitivePortableLabels = detectSensitivePortableLabels(value);
  if (sensitivePortableLabels.length) {
    add(report, "errors", `${label} must not include credential-looking label metadata: ${sensitivePortableLabels.join(", ")}.`);
  }
  const sensitiveRecordIds = detectSensitiveRecordIds(value);
  if (sensitiveRecordIds.length) {
    add(report, "errors", `${label} must not include credential-looking record IDs: ${sensitiveRecordIds.join(", ")}.`);
  }
  const sensitiveTermNotes = detectSensitiveTermNotes(value);
  if (sensitiveTermNotes.length) {
    add(report, "errors", `${label} must not include credential-looking termbase notes: ${sensitiveTermNotes.join(", ")}.`);
  }
  const sensitiveTmOrigins = detectSensitiveTmOrigins(value);
  if (sensitiveTmOrigins.length) {
    add(report, "errors", `${label} must not include credential-looking TM origin metadata: ${sensitiveTmOrigins.join(", ")}.`);
  }
  const sensitiveAiSuggestionMetadata = detectSensitiveAiSuggestionMetadata(value);
  if (sensitiveAiSuggestionMetadata.length) {
    add(report, "errors", `${label} must not include credential-looking AI suggestion metadata: ${sensitiveAiSuggestionMetadata.join(", ")}.`);
  }
  ...
  if (runtimeOnlyFields.length) {
    add(report, "errors", `${label} must not include browser-only handles or runtime objects: ${runtimeOnlyFields.join(", ")}.`);
  }
}
...
function validateSourceAssetConsistency(report, pkg, documents) {
  ...
  pkg.sourceAssets.forEach((asset, index) => {
    ...
    if (seenAssetIds.has(asset.id)) add(report, "warnings", `Source assets contain duplicate document ID: ${asset.id}.`);
  ...
  documents.forEach((documentInfo) => {
    ...
    const docxStructure = pkg.project?.docxStructures?.[documentInfo.id] || (documentInfo.type === "docx" ? pkg.project?.docxStructure : null);
    const localizationStructure = pkg.project?.localizationStructures?.[documentInfo.id];
    const hasOriginal = Boolean(docxStructure?.docxPackageBase64 || hasOriginalLocalizationStructure(localizationStructure));
    ...
    if (asset.originalAvailable && !hasOriginal) {
      add(report, "warnings", `${name} source asset claims original availability, but reconstruction data is missing.`);
    }
...
function validateProjectPackage(pkg) {
  ...
  if (legacy) add(report, "warnings", "Legacy CatHan package accepted. Re-export it as a LoopCAT package after import.");
  ...
  } else if (pkg.schemaVersion > MAX_SCHEMA_VERSION) {
    add(report, "errors", `Schema version ${pkg.schemaVersion} is newer than this LoopCAT build supports. Update LoopCAT before importing this package.`);
  }
  if (!isIsoDate(pkg.exportedAt)) add(report, legacy ? "warnings" : "errors", "Package export timestamp is missing or invalid.");

  if (!isObject(pkg.packageMetadata)) {
    add(report, strictBucket, "Package metadata is missing.");
  } else {
    if (!legacy && pkg.packageMetadata.format !== PACKAGE_FORMAT) add(report, "errors", `Package format must be ${PACKAGE_FORMAT}.`);
    if (!legacy && pkg.packageMetadata.packageVersion !== pkg.version) add(report, "errors", "Package metadata version must match package version.");
    if (!legacy && pkg.packageMetadata.contractVersion !== CONTRACT_VERSION) add(report, "errors", `Package contract must be ${CONTRACT_VERSION}.`);
  ...
  if (!isObject(pkg.project)) {
    ...
    if (!isIsoDate(pkg.project.createdAt)) add(report, "warnings", "Project creation timestamp is missing or invalid.");
  ...
  if (segments.length) add(report, "preserved", `${segments.length} segment${segments.length === 1 ? "" : "s"} included.`);
  if (emptyTargets) add(report, "warnings", `${emptyTargets} target segment${emptyTargets === 1 ? "" : "s"} are empty.`);
  ...
  documents.forEach((documentInfo, index) => {
    ...
    if (documentInfo.id) {
      if (documentIds.has(documentInfo.id)) add(report, strictBucket, `Duplicate document ID in project manifest: ${documentInfo.id}.`);
  ...
  segments.forEach((segment, index) => {
    ...
    if (segment.documentId && hasDocumentManifest && !documentIds.has(segment.documentId)) {
      add(report, strictBucket, `Segment ${segment.id || index + 1} refers to document ${segment.documentId}, but that document is not listed in the project manifest.`);
    }
    ...
    if (typeof segment.source !== "string") add(report, "errors", `Segment ${segment.id || index + 1} source text must be a string.`);
    if (segment.targetHistory !== undefined && !Array.isArray(segment.targetHistory)) add(report, "warnings", `Segment ${segment.id || index + 1} target history must be an array.`);
    if (!Number.isFinite(segment.index)) add(report, "warnings", `Segment ${segment.id || index + 1} index is missing or invalid.`);
    ...
    if (!isIsoDate(segment.createdAt)) add(report, "warnings", `Segment ${segment.id || index + 1} creation timestamp is missing or invalid.`);
    if (!isIsoDate(segment.updatedAt)) add(report, "warnings", `Segment ${segment.id || index + 1} update timestamp is missing or invalid.`);
  });
  ...
  validDocuments.forEach((documentInfo) => {
    ...
    if (type === "idml" && pkg.project.localizationStructures?.[documentInfo.id] && !pkg.project.localizationStructures[documentInfo.id].packageBase64) {
    ...
    if (pkg.project.localizationStructures?.[documentInfo.id] && documentSegments.length) {
      ...
      if (requirement && missing.length) {
        add(report, "warnings", `${documentInfo.name || requirement.label} has ${missing.length} segment${missing.length === 1 ? "" : "s"} missing ${requirement.issue}.`);
      }
    }
  });
  ...
  if (pkg.resources && !isObject(pkg.resources)) add(report, "warnings", "Package resources must be an object when present.");
  ...
  if (pkg.resourceReferences && !Array.isArray(pkg.resourceReferences)) add(report, "warnings", "Resource references must be an array.");
  if (pkg.sourceAssets && !Array.isArray(pkg.sourceAssets)) add(report, "warnings", "Source assets must be an array.");
  if (pkg.activityEvents && !Array.isArray(pkg.activityEvents)) {
    ...
    pkg.activityEvents.forEach((event, index) => {
      ...
      if (hasSensitiveTextValue(event?.summary)) {
        add(report, "errors", `Activity event ${event?.id || index + 1} summary must not include credential-looking text.`);
      }
...
function validateBackupFile(data) {
  ...
  } else if (data.schemaVersion > MAX_SCHEMA_VERSION) {
    add(report, "errors", `Backup schema version ${data.schemaVersion} is newer than this LoopCAT build supports. Update LoopCAT before restoring this backup.`);
  }
  ...
  [
    ...
    if (data[field] === undefined) {
      add(report, field === "activityEvents" ? "warnings" : "errors", `${label} are missing.`);
  ...
  (Array.isArray(data.projects) ? data.projects : []).forEach((project, index) => {
    ...
    if (project.documents === undefined) {
      ...
      project.documents.forEach((documentInfo, documentIndex) => {
        if (!isObject(documentInfo)) {
          add(report, "errors", `Project ${projectLabel} document ${documentIndex + 1} manifest entry must be an object.`);
          return;
        }
        requireString(report, documentInfo, "id", `Project ${projectLabel} document ${documentIndex + 1} ID`);
        if (documentInfo.id) {
          if (documentIds.has(documentInfo.id)) add(report, "errors", `Duplicate document ID in project manifest: ${documentInfo.id}.`);
          documentIds.add(documentInfo.id);
        }
        requireString(report, documentInfo, "name", `Project ${projectLabel} document ${documentIndex + 1} name`, "warnings");
        requireString(report, documentInfo, "type", `Project ${projectLabel} document ${documentIndex + 1} type`, "warnings");
      });
  ...
  (Array.isArray(data.segments) ? data.segments : []).forEach((segment, index) => {
    ...
    if (segment.projectId && segment.documentId && projectDocumentIds.has(segment.projectId) && !projectDocumentIds.get(segment.projectId).has(segment.documentId)) {
      add(report, "errors", `Segment ${segment.id || index + 1} refers to document ${segment.documentId}, but that document is not listed in the restored project manifest.`);
    }
  });
  (Array.isArray(data.activityEvents) ? data.activityEvents : []).forEach((event, index) => {
    ...
    if (event.projectId && !projectIds.has(event.projectId)) {
      add(report, "errors", `Activity event ${event.id || index + 1} belongs to a project not present in the backup.`);
    }
    if (hasSensitiveTextValue(event.summary)) {
      add(report, "errors", `Activity event ${event.id || index + 1} summary must not include credential-looking text.`);
    }
...
function hasInvalidXmlCharacters(text) {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    if ((code >= 0x00 && code <= 0x08) || code === 0x0b || code === 0x0c || (code >= 0x0e && code <= 0x1f) || code === 0xfffe || code === 0xffff) {
      return true;
    }
...
function isXmlDeliveryFormat(format, documentInfo) {
  ...
  return [
    "docx", "bilingual-docx", "docm", "dotx", "dotm", "xlsx", "xlsm", "xltx", "xltm", "pptx", "pptm", "ppsx", "ppsm", "potx", "potm",
    "odp", "otp", "ods", "ots", "odt", "ott",
    "xlf", "xliff", "sdlxliff", "xml", "xhtml", "dita", "txml", "ttx", "xini", "resx", "wix", "ts", "idml", "icml"
  ].includes(value);
}
...
function compactHtmlAttributeValue(value) {
  return decodeHtmlAttributeEntities(value).replace(/[\u0000-\u001f\u007f\s]+/g, "").toLowerCase();
}
...
function hasUnsafeHtmlMarkup(text) {
  ...
  for (const match of String(text || "").matchAll(pattern)) {
    ...
    if (/\bstyle\s*=\s*(?:"[^"]*(?:expression\s*\(|javascript:)|'[^']*(?:expression\s*\(|javascript:)|[^\s>]*(?:expression\s*\(|javascript:))/i.test(attrs)) return true;
    if (/\b(?:href|src|xlink:href|formaction)\s*=\s*(?:"\s*javascript:|'\s*javascript:|javascript:)/i.test(attrs)) return true;
    if (/\b(?:href|src|xlink:href|formaction)\s*=\s*(?:"\s*data\s*:\s*(?:text\/html|image\/svg\+xml)|'\s*data\s*:\s*(?:text\/html|image\/svg\+xml)|data\s*:\s*(?:text\/html|image\/svg\+xml))/i.test(attrs)) return true;
    for (const attr of htmlAttributeEntries(attrs)) {
      ...
      if (URL_HTML_ATTRIBUTES.has(attr.name) && (compactValue.startsWith("javascript:") || compactValue.startsWith("vbscript:") || isActiveDataUrl(compactValue))) return true;
    }
  }
  return false;
}
...
function validateExportReadiness({ project, segments = [], documentInfo = null, format = "project", terms = [], exportPlan = null, structure = null }) {
  ...
  if (unsafeHtmlIssues) add(report, "risky", `${unsafeHtmlIssues} segment${unsafeHtmlIssues === 1 ? "" : "s"} contain unsafe HTML markup.`);
  if (forbiddenTermIssues) add(report, "risky", `${forbiddenTermIssues} segment${forbiddenTermIssues === 1 ? "" : "s"} contain forbidden terminology.`);
  ...
  if (documentInfo && LOCALIZATION_TYPES.has(documentType) && !localizationStructure) {
    ...
    else if (documentType === "xlf" || documentType === "xliff" || documentType === "sdlxliff") add(report, "errors", "XLIFF reconstruction source data is missing.");
    else add(report, "errors", `${documentInfo.name || "Current file"} original localization structure metadata is missing.`);
  }
  if ((format === "idml" || documentType === "idml") && documentInfo && localizationStructure && !localizationStructure.packageBase64) {
    add(report, "errors", "IDML reconstruction package data is missing.");
  }
  if ((format === "xlf" || format === "xliff" || format === "sdlxliff" || documentType === "xlf" || documentType === "xliff" || documentType === "sdlxliff") && documentInfo && localizationStructure && !localizationStructure.source) {
...
function reportCount(report) {
  return ["errors", "warnings", "risky", "simplified", "skipped"].reduce((sum, key) => sum + (report?.[key]?.length || 0), 0);
}
